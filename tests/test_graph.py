...
from heff.purify import Pulse, entropy, greedy, info_gain, outcome_split, pulse_library, run
...
def test_outcome_split_conserves_probability_and_selective_pulse_purifies():
    ...
    p = Pulse(("pi", "pi"), 0.0, np.array([0, 1]), np.array([2, 2]), np.array([1.0, 0.5]))
    ...
    one = Pulse(("pi", "pi"), 0.0, np.array([1]), np.array([2]), np.array([1.0]))
