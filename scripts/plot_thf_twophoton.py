...
for B in B_PANELS:
    ...
    np.savez_compressed(OUT / f"matrices_B{B:g}G.npz", **{f"{a}_{b}": t.amp for (a, b), t in mats.items()},
             evals=eig.evals, rows=rows, cols=cols, freqs=next(iter(mats.values())).freqs)
    with open(OUT / f"lines_B{B:g}G.csv", "w") as f:
        f.write("eps1,eps2,i,j,freq_MHz,strength,a_J,a_F1,a_F,a_ef,a_mF,a_parity,b_J,b_F1,b_F,b_ef,b_mF,b_parity\n")
        for (a, b), t in mats.items():
            for r in t.lines():
                f.write(f"{a},{b},{r['i']},{r['j']},{r['freq_MHz']:.6f},{r['strength']:.6e},"
                        f"{r['a_J']:g},{r['a_F1']:g},{r['a_F']:g},{r['a_ef']},{r['a_mF']:+g},{r['a_parity']:+d},"
                        f"{r['b_J']:g},{r['b_F1']:g},{r['b_F']:g},{r['b_ef']},{r['b_mF']:+g},{r['b_parity']:+d}\n")
...
Files: heatmaps_B*.png, matrices_B*.npz (complex amps keyed pol1_pol2, freqs = E_final - E_initial in MHz), labels_B*.csv,
lines_B*.csv (every |M_ij|^2 > 1e-12 per pair: i, j index rows/cols, freq_MHz = E_final - E_initial, strength, initial a_* and final b_* labels),
curves_*.png, sweep_*.npz, channels.npz.
