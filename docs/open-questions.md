...
| `OPEN-2` | **Resolved.** The package uses Brown's e/f rule, under which e is the component of parity `(-1)^J`, and it places e BELOW f at every J. The ordering follows Petrov and Skripnikov's model: the `1Sigma+` state 314 cm-1 above mixes with the e component alone, because an `Omega = 0+` state carries only e levels, and pushes it down. That is the same mixing their Omega-doubling fit rests on, and it agrees with Gresh 2016 Table 1's `k'' > 0` on the `Omega = 0+` bands. Ng's experiment did not determine the ordering (thesis Fig. 1.4 caption, p.30), so his schematics are not evidence either way. | Applying the thesis's `S=1/2` specialization at `S=1` would invert every e/f label. The ordering fixes the sign, not the magnitude, of every predicted parity-doublet differential, `Delta g` included. |
| `OPEN-3` | **Resolved for the implementation.** The package uses `+G_zz mu_B (J.n)(n.B)` for the axial Zeeman component, which reproduces Ng's printed g-factor relation and measured magnitude. Ng thesis Eq. C.6 prints the opposite sign. | The source-level discrepancy remains worth author confirmation, but the implemented branch is fixed and tested. |
...
| `OPEN-7` | A separate rotational g-factor `g_r` is not carried. Its physics is absorbed into `G_perp = (G_xx + G_yy)/2`: the perpendicular part of `mu_B B.G.J` is the term that couples to the rotational angular momentum, and its `B.J` piece is exactly a rotational g-factor. The value is Petrov and Skripnikov's second-order estimate, not a fit. | Predicted J = 2--4 g-factors now carry the uncertainty of that estimate rather than a missing term. `G_perp` is an estimate, so the J dependence of `g` remains the model's least-constrained prediction. |
...
| `OPEN-10` | **Resolved.** The Zeeman operator is the full body-frame tensor, `H_Z = mu_B B.G.J` with `G = diag(G_xx, G_yy, G_zz)`. Its `Delta Omega = +/-2` part, with coefficient `G_Delta = (G_xx - G_yy)/2`, is parity-dependent and splits the two components of every Omega doublet: `g_e - g_f = 2 G_Delta` before projection onto F. Only `G_zz + G_perp` is measured; `G_xx` and `G_yy` separately are Petrov and Skripnikov second-order estimates. | The model now generates a zero-field differential g-factor, `+2.74e-4` at J = 1, F = 3/2. Its MAGNITUDE inherits roughly 20 percent from the estimate of `G_Delta`; its SIGN is set by the doublet ordering of `OPEN-2` together with the fact that only e mixes with the `Omega = 0+` states. |
...
| `OPEN-13` | Gresh's printed `k''` sign depends on upper-state branch bookkeeping. | The package takes the Omega-doublet SPLITTING from the microwave result. The ORDERING comes from Petrov and Skripnikov's `1Sigma+` mixing (`OPEN-2`), with Gresh's `k'' > 0` on the `Omega = 0+` bands as an independent but branch-dependent agreement, not as the primary evidence. |
