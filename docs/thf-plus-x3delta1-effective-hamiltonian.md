...
Note there is **no −Ω² term**: this is the case-(c) convention of Ng Ch. 2.4 Eq. 2.2a (p. 37), `E(J) = B J(J+1), J ≥ Ω`, with the Ω-dependent constant absorbed into the band origin. The rotational-only J=1→2 interval is 4B, but the registered centrifugal term makes it 4B−32D. The stored B is Ng's interval divided by four, not a joint refit: with D=3.897 kHz this shifts the calculated rotational interval down by 124.704 kHz. A consistent fit is needed before treating this combination as a precision reproduction of the measured interval.
...
|---|---|---|---|---|---|
| **B₀** | 4B = 29.09733(4) | GHz | B₀ = 7.2743325(100) GHz = 0.2426456 cm⁻¹ `[29.09733/4; interval-only, not D-aware refit]` | Ng 2022 §II B p. 2; Ng thesis §4.1.1 p. 78 | derived |
...
**Two *committed* gates now close the same hole at I_Th = 5/2**, so it no longer rests on scratch scripts. Both were re-measured under the same upper-pair transposition applied to every 6j `heff.elements_c2` evaluates, with `heff.wigner.w6j`'s raise wrapped as `0.0` (2026-09-06): the E1 sum rule of `tests/test_spectra_c2.py` (`test_B7_sum_rule_still_holds_in_the_two_spin_basis`) goes from **1.0000 to 0.0**, and `tests/test_twophoton.py`'s rank-K sum rule V26 (`test_rank_K_sum_rule_and_reciprocity`) from **1.0 to 0.0** at its first (J = 1, F₁ = 7/2, F = 4) entry. Both run on the real ²²⁹ThF⁺ two-spin basis, where `F₁ ≠ J`.

**R15 — what the two transposition rows do and do not show.** Both transposition rows act by killing the symbol — the swapped triads mix integer and half-integer angular momenta, so the corrupted element is identically zero (`heff.wigner.w6j` raises on such a triad; the tests wrap the raise as 0.0) and max |dev| is just max |correct|. They show the 6j's are load-bearing, not that their column order is right; the column-order test is a transposition that stays inside valid triads (exchanging the bra's and the ket's totals — argument slots 2 and 4 — measured at max |dev| = 0.5477 over `elements_c2.axial_geometry` at `k = 1, q = 0` on `thf_spec('229', J_max=2)`, 2026-09-06), and, unlike the two rows above, it leaves the operator ALIVE: `tests/test_twophoton_closure.py` asserts `max |T| > 0.1` under it before reading any deviation. Read per component: at `p = ±1` the deviation is the whole element (0.5477 of 0.5477), at `p = 0` it is 0.4082 of 0.5000.
...
**Correction to [TH] §4.5** `[derived]`: that section quotes `0.13–0.37 × A∥^F/[J(J+1)]` ≈ 1.3–3.8 MHz, i.e. **low by one factor of `J(J+1)`** — the `1/[J(J+1)]` of the operator is already inside the numbers printed in the table above (its header reads `⟨F₁′|O|F₁⟩/[J(J+1)]` with `O = T¹(J)·T¹(I_F)`), so multiplying by it again double-counts. §9.3 is self-consistent and is the version to code. The two ranges are over **different element sets**, which is why their lower endpoints differ (0.32 here, 0.13 there): the §9.3 numbers above are the J = 1 elements only, while [TH]'s lower endpoint is a **J = 2** element — the ΔF₁ = ±1 magnitudes computed from `heff` on `thf_spec('229', J_max=4)` are {0.3194, 0.3742} at J = 1 and {0.1309, 0.1750, 0.2000, 0.2079} at J = 2 in units of `A∥^F` (2026-09-06), so 0.13 is the smallest J = 2 entry and 0.37 the largest J = 1 one. The upper endpoints agree because both are the same J = 1 element, 0.374166. With the corrected elements the second-order shift is `(7.52 MHz)²/1905 MHz ≈ 30 kHz` for the F₁ = 3/2 ↔ 5/2 pair and `(6.42)²/2667 ≈ 15 kHz` for 5/2 ↔ 7/2, against [TH]'s ≲ 25 kHz — **the conclusion (F₁ is a good quantum number) is unchanged**, and the digest is deliberately left as written; the correction lives here.
...
Audited 2026-09-15: docs/lit/2026-09-15-twophoton-operator-audit.md
...
S9.5.1(3)  polarisation dyad: conjugate + slot order
  ...
  (d) Delta m_F = P reachable, LADDER reading (both photons absorbed) T_eff = (d.eps2)(d.eps1)/Delta
      eps1=sigma+ eps2=sigma+ : Delta m_F = [2]
      eps1=sigma+ eps2=sigma- : Delta m_F = [0]
      eps1=sigma- eps2=sigma- : Delta m_F = [-2]
  (e) same, RAMAN reading, available as `reading='raman'`  T_eff = (d.eps2*)(d.eps1)/Delta
...
This is the algebraic content of [2γ] §3.3's table (Ω = +1 → −1 requires an Ω = 0 intermediate, because each E1 leg has `|q_i| ≤ 1` and `ΔΩ = q₁ + q₂`). The K = 0 channel matrix is exactly the identity on any basis (probe and gate `test_K0_is_identity`, 2026-09-15); transitions are carried by K = 2 alone.
...
**The alternative, recorded for comparison** `[derived]`: taking `μ(²²⁷) = μ(²²⁹) = 0.366 μ_N` with `I = 1/2` gives `g_N = 0.732` and `A∥ = −10 408 × 0.732 = −7619 MHz ≈ −7.62 GHz` — same order as the Schmidt value in magnitude but **opposite in sign** and 5× smaller. These historical assumptions disagree in sign. The direct 2024 nuclear-theory alternative above gives +1.790 GHz, far below either magnitude; the spread between these models is not a confidence interval.

**Structural algebra and consequences of the historical placeholders** `[derived]`. At `I_Th = 1/2` the Th hyperfine has the same algebraic shape as the ¹⁹F structure already coded (§9.2), with `F₁ = J ± 1/2` and splitting `A∥^Th (2J+1)/[2J(J+1)]`, i.e. `0.75 A∥^Th` at J = 1. At the Schmidt value that is **+29.9 GHz at J = 1, against `4B = 29.1 GHz`** — the Th hyperfine splitting is as large as the whole J = 1 → 2 rotational interval. The ΔJ = ±1 element of B&C (9.51) at (J = 2 ← 1, F₁ = 3/2, Ω = 1) is `−0.433013 × A∥^Th = −17.2 GHz`, i.e. **0.59 × the rotational spacing**: J is not even approximately a good quantum number for ²²⁷ThF⁺ at this A∥, second-order perturbation theory is meaningless, and the J_max truncation must be re-tested for this isotopologue specifically (OPEN-22). Even at the −7.62 GHz alternative the ΔJ = ±1 element is 3.30 GHz (0.113 × the rotational spacing), i.e. ~340× the 8.7–9.7 MHz ²³²ThF⁺ elements of §2.5. For the 2024 moment prediction, the same element would instead be about −0.775 GHz (0.027 of the rotational interval). Truncation must be checked for the parameter set actually used; the Schmidt strong-mixing conclusion does not transfer unchanged to this alternative.

**OPEN-20 remains a scientific uncertainty.** A deformed-nucleus calculation is now available, but its 227Th moment has no calibrated uncertainty and the current result omits Coriolis/collective mixing corrections. A measurement or a validated nuclear-model uncertainty assessment would constrain the molecular prediction. Kälber et al., Z. Phys. A **334**, 103 (1989), remains unread in full; its abstract is not evidence of an absent 227Th measurement.
