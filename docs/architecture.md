...
`graph.py` turns a set of `TransitionMatrix` objects at one field point into a networkx `MultiDiGraph` (nodes = eigenstates with labels and energy, edges = lines tagged with strength, frequency and polarization pair) and supplies Boltzmann populations. `purify.py` reads that graph as a pulse library for quantum-logic state purification: each candidate pulse is a polarization pair, a center frequency and a bandwidth, applied as a sideband pi pulse with a binary phonon readout, and the conditional state is a classical belief vector updated per outcome; `greedy` picks the pulse of largest expected information gain. `pulse_library(closed=True)` restricts to closed two-level pulses, dropping open Zeeman and diagonal ladders that the pi-pulse model does not describe; `propagator_library` replaces the top-hat pi-pulse tables with the exact propagator of each pulse on its addressed states times phonons 0..nph (detunings, ladders, displacement drives), stored in the same `Pulse` transfer-table form. `plot_transition.level_panels` draws the graph per J block. `scripts/purify_thf.py` is the deck.
